...
# Super breakpoint
def sbr(debugger,elf_file, command, result, internal_dict):

    #用户是否输入了地址参数
    if not command or not elf_file:
        print >>result, 'Please input the address or elf_file_name!'
        return

    ASLR = get_ASLR(elf_file)
    if ASLR:
        #如果找到了ASLR偏移，就设置断点
